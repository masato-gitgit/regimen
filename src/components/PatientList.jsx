          ...
          <div>
            ...
            <div className="card" style={{ marginTop: '20px' }}>
              ...
              <div className="card-body">
                <div className="form-row" style={{ alignItems: 'flex-end' }}>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    <input
                      ...
                      step="0.1"
                      className="form-control"
                      placeholder={`現在: ${selectedPatient.weight} kg`}
                      value={weightInput}
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    <input
                      ...
                      step="0.1"
                      className="form-control"
                      placeholder={`現在: ${selectedPatient.height} cm`}
                      value={heightInput}
                ...
                {/* 用量変更プレビュー */}
                {showDosePreview && weightInput && parseFloat(weightInput) > 0 && selectedPatient.activeRegimen && (() => {
                  const previewWeight = parseFloat(weightInput);
                  const previewHeight = heightInput ? parseFloat(heightInput) : selectedPatient.height;
                  const previewBSA = calculateBSA(previewHeight, previewWeight);
                  const drugs = selectedPatient.activeRegimen.drugs || [];
                  const calcDrugs = drugs.map(drug => {
                    let oldDose, newDose;
                    if (drug.doseType === 'bsa') {
                      oldDose = formatDose(drug.doseValue * selectedPatient.bsa, drug.name);
                      newDose = formatDose(drug.doseValue * previewBSA, drug.name);
                    } else if (drug.doseType === 'weight') {
                      oldDose = formatDose(drug.doseValue * selectedPatient.weight, drug.name);
                    ...
                    <div style={{ marginTop: '16px', borderTop: '1px solid var(--color-border)', paddingTop: '14px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'var(--color-secondary)', fontWeight: '600' }}>
                          BSA: {selectedPatient.bsa} m² → {previewBSA} m²
                        </span>
                      </div>
